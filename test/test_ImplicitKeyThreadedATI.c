...
int main(int argc, char* argv[]) {
    struct ImKeyAti* tree = newImKeyAti();
    ...
    for (int i = 0; i < DATA_SIZE; i++) {
        ...
        *(ptr++) = random;
        ...
        insertImKeyAtiNode(tree, (const tree_key_t*)&random, NULL);
    ...
    {
        struct ImKeyAtiNode* node = getFirstImKeyAtiNode(tree);
        size_t cnt = 0;
        while (node) {
            PRINT_F("%04d   %06llu \n", cnt, (uint64_t)node->key_);
            size_t index = -1;
            findImKeyAtiNodeWithIndex(tree, node->key_, &index);
            if (findImKeyAtiNodeByIndex(tree, cnt) != node)
    ...
    if (1) {
        ...
        for (int i = 0; i < DATA_SIZE; i++) {
            test_data_type_t temp_data = *(data_array + i);
            removeImKeyAtiNode(tree, &temp_data);
        ...
        struct AtiNode* node = getFirstImKeyAtiNode(tree);
        walkValidate(tree->top_node_);
        int cnt = 0;
        while (node && cnt < 20) {
            node = node->next_;
            removeImKeyAtiNode(tree, &node->previous_->key_);
